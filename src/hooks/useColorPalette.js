...
export const useColorPalette = () => {
    ...
    const generatePalette = useCallback((newBaseColor, newHarmony) => {
        ...
        try {
            const newPalette = ColorUtils.generateHarmony(colorToUse, harmonyToUse, isDarkMode);
    ...
    const updateColor = useCallback((role, color) => {
    ...
    // Lock/unlock a color
    const toggleLock = useCallback((role) => {
        dispatch(toggleColorLock(role));
    }, [dispatch]);

    // Check if a color is locked
    const isColorLocked = useCallback((role) => {
        return lockedColors.has(role);
    }, [lockedColors]);

    // Randomize the palette
    const randomize = useCallback((preserveLocked = true) => {
    ...
    const getColor = useCallback((role, fallback = '#000000') => {
        return palette[role] || fallback;
    }, [palette]);

    // Get all colors as an array for easy iteration
    const colorsArray = useMemo(() => {
        return Object.entries(palette).map(([role, color]) => ({
            role,
            color,
    ...
    const generateVariations = useCallback((steps = 3) => {
        const variations = [];

        Object.entries(palette).forEach(([role, color]) => {
            if (!lockedColors.has(role)) {
                const { tints, shades } = ColorUtils.generateTintsAndShades(color, steps);
                variations.push({
                    role,
                    baseColor: color,
                    tints,
    ...
    const exportPalette = useCallback((format = 'object') => {
        switch (format) {
            case 'css':
