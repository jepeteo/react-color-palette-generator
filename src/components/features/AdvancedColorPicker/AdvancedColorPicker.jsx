...
import { setBaseColor, randomizePalette } from '../../../store/slices/paletteSlice';
...
function AdvancedColorPicker() {
        ...
        if (pickerMode === 'wheel') {
            ...
            for (let angle = 0; angle < 360; angle += 1) {
                for (let r = 0; r < radius; r += 1) {
                    ...
                    const lightness = 50;
                    const color = ColorUtils.hslToHex({ h: angle, s: saturation, l: lightness });
            ...
            for (let x = 0; x < width; x++) {
                for (let y = 0; y < height; y++) {
                    ...
                    const color = ColorUtils.hslToHex({ h: hsl.h, s: saturation, l: lightness });
    ...
    return (
        <Card title="Advanced Color Picker" subtitle="Precise color selection with multiple color spaces">
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                        <div className="text-lg font-mono text-white">{selectedColor}</div>
                        <div className="text-sm text-white/60">
                            RGB({rgb.r}, {rgb.g}, {rgb.b}) • HSL({Math.round(hsl.h)}°, {Math.round(hsl.s)}%, {Math.round(hsl.l)}%)
                        </div>
                    </div>
                ...
                <div className="flex gap-2">
                    {[
                        ...
                        <button
                            ...
                            className={`px-3 py-2 rounded-lg text-sm transition-all ${pickerMode === mode.id
                ...
                {pickerMode === 'wheel' && (
                    <div className="space-y-4">
                        <canvas
                            ...
                            className="border border-white/20 rounded-lg cursor-crosshair mx-auto block"
                        />
                ...
                <div>
                    ...
                    <input
                        ...
                        className="w-full bg-white/10 border border-white/20 rounded px-3 py-2 text-white font-mono focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                        placeholder="#000000"
                    />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    >
                        <div className="text-white/60 text-xs mb-1">HEX</div>
                        <div className="text-white font-mono text-sm">{selectedColor}</div>
                    </div>
                    <div
                        className="bg-white/5 rounded-lg p-3 cursor-pointer hover:bg-white/10 transition-colors"
                        onClick={() => copyToClipboard(`rgb(${rgb.r}, ${rgb.g}, ${rgb.b})`, 'RGB')}
                    >
                        <div className="text-white/60 text-xs mb-1">RGB</div>
                    ...
                    <div
                        className="bg-white/5 rounded-lg p-3 cursor-pointer hover:bg-white/10 transition-colors"
                        onClick={() => copyToClipboard(`hsl(${Math.round(hsl.h)}, ${Math.round(hsl.s)}%, ${Math.round(hsl.l)}%)`, 'HSL')}
                    >
                        <div className="text-white/60 text-xs mb-1">HSL</div>
                        <div className="text-white font-mono text-sm">
