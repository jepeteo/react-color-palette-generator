...
function GradientGenerator() {
  ...
  const gradientTypes = {
    linear: {
      name: 'Linear',
      icon: '↗️',
      description: 'Straight line gradients',
    },
  ...
  const generateGradientColors = useCallback(
    (startColor, endColor, steps, colorSpace, easing) => {
      try {
        const colors = [];
        ...
        if (colorSpace === 'hsl') {
          ...
          for (let i = 0; i < steps; i++) {
            ...
            const r = Math.round(
              interpolate(startRgb.r, endRgb.r, easedProgress),
            );
            const g = Math.round(
              interpolate(startRgb.g, endRgb.g, easedProgress),
            );
            const b = Math.round(
              interpolate(startRgb.b, endRgb.b, easedProgress),
            );
  ...
  return (
    <Card
      ...
      subtitle="Create beautiful gradients and extract color palettes"
    >
      <div className="space-y-6">
        ...
        {palette.colors && palette.colors.length >= 2 && (
          <div className="space-y-4">
            <div
              className="relative h-32 overflow-hidden rounded-lg border border-white/20"
              style={{
                background:
                  gradientType === 'linear'
            ...
            {showCode && (
              <div className="rounded-lg bg-black/30 p-4">
                <div className="mb-2 flex items-center justify-between">
                  <h4 className="text-sm font-medium text-white">CSS Code</h4>
          ...
          <div className="space-y-4">
            ...
            {gradientType === 'linear' && (
              <div>
                ...
                <select
                  ...
                  className="w-full rounded border border-white/20 bg-white/10 px-2 py-1 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                >
                  {directions.map((dir) => (
                    <option
                      key={dir.value}
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded border border-white/20 bg-white/10 px-2 py-1 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50"
              >
                <option value="rgb" className="bg-gray-800">
                  RGB
                </option>
                <option value="hsl" className="bg-gray-800">
                  HSL
                </option>
              </select>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
            {Object.entries(presetGradients).map(([name, colors]) => (
              <button
                ...
                className="group relative h-16 overflow-hidden rounded-lg border border-white/20 transition-all duration-200 hover:border-white/40"
                style={{
        ...
        <div className="rounded-lg border border-purple-500/20 bg-purple-500/10 p-4">
          <h5 className="mb-2 flex items-center font-medium text-purple-300">
          ...
          <ul className="space-y-1 text-sm text-purple-200/80">
            ...
            <li>
              • Try different easing functions for unique transition curves
            </li>
            <li>
              • Combine multiple gradients with blend modes for complex effects
            </li>
          </ul>
        </div>
      </div>
