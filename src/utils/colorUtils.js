...
export class ColorUtils {
  ...
  static generateHarmony(
    baseColor,
    harmonyType = HARMONY_TYPES.TRIADIC,
    isDarkMode = false,
  ) {
    try {
      ...
      const generators = {
        ...
        [HARMONY_TYPES.SPLIT_COMPLEMENTARY]: () =>
          this.generateSplitComplementary(base),
        ...
        [HARMONY_TYPES.DOUBLE_SPLIT_COMPLEMENTARY]: () =>
          this.generateDoubleSplitComplementary(base),
      };

      const colors =
        generators[harmonyType]?.() || generators[HARMONY_TYPES.TRIADIC]();

      // Add appropriate text and background colors
      return {
        ...colors,
        [PALETTE_ROLES.TEXT]: this.getOptimalTextColor(
          colors[PALETTE_ROLES.BACKGROUND] || colors[PALETTE_ROLES.PRIMARY],
          isDarkMode,
        ),
